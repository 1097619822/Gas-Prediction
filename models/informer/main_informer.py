...
import os
import torch
from ChangeInformerData import ChangeInformerData
from InformerResult import InformerResult
from exp.exp_informer import Exp_Informer

#['--root_path=d','--data_path=ETTh3.csv','--target=OT','--freq=d','--checkpoints=./checkpoints/','--seq_len=96','--label_len=48','--pred_len=24','--enc_in=7','--dec_in=7','--n_heads=8']

def InformerPred(params):
    parser = argparse.ArgumentParser(description='[Informer] Long Sequences Forecasting')
    parser.add_argument('--model', type=str, default='informer',help='model of experiment, options: [informer, informerstack, informerlight(TBD)]')
    parser.add_argument('--data', type=str, default='custom', help='data')
    parser.add_argument('--root_path', type=str, default='E:\oilgas', help='root path of the data file')
    ...
    data_parser = {   
        'custom':{'data':'data.csv','T':'LABEL','M':[6,6,6],'S':[1,1,1],'MS':[6,6,1]},
        'ETTh1':{'data':'59-24X.csv','T':'LABEL','M':[6,6,6],'S':[1,1,1],'MS':[6,6,1]},
